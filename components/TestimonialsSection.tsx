...
function TestCard({ t }: { t: typeof TESTIMONIALS[0] }) {
  return (
    <div className="rounded-2xl p-6 w-72 flex-shrink-0 mx-2 feature-card"
      ...
      <div className="flex items-center gap-3 mb-3">
        ...
        <div>
          <p className="font-display font-bold text-sm" style={{ color: "var(--text-heading)" }}>{t.name}</p>
          <p className="text-xs font-mono" style={{ color: "var(--text-faint)" }}>{t.handle}</p>
        </div>
        <div className="ml-auto flex gap-0.5">
          {Array.from({length:t.stars}).map((_,i) => <span key={i} className="text-amber-400 text-xs">★</span>)}
        </div>
      </div>
      <p className="text-sm leading-relaxed overflow-clip" style={{ color: "var(--text-body)" }}>&ldquo;{t.text}&rdquo;</p>
    </div>
  );
}
...
export default function TestimonialsSection() {
  ...
  return (
    <section ref={ref} className="relative py-28 overflow-hidden section-base">
      ...
      <div className="text-center mb-14 px-6 reveal">
        ...
        <h2 className="font-display font-extrabold text-4xl md:text-5xl text-heading">
